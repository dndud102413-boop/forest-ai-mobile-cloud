...
_bundle_error = None
if _bundle_url and os.environ.get("FOREST_RECO_ENABLE_BUNDLE") == "1":  # 부팅 안정화: 기본 데모모드(170MB 다운로드 스킵). 실데이터는 FOREST_RECO_ENABLE_BUNDLE=1로 켬
    try:
...
with tab_gps:
    st.markdown("<div class='input-card'>'현재 위치 가져오기'를 누르고 브라우저 위치 권한을 허용하면 현재 좌표가 자동 입력됩니다.</div>",
                unsafe_allow_html=True)
    st.caption("⚠ 브라우저 GPS는 HTTPS 환경에서 동작합니다. (이 사이트는 HTTPS라 사용 가능)")
    if st.button("📍 현재 위치 가져오기", key="gps_btn"):
        ss["_gps_on"] = True
    if ss.get("_gps_on"):
        try:
            from streamlit_js_eval import get_geolocation
            _loc = get_geolocation()
            _c = (_loc or {}).get("coords") if isinstance(_loc, dict) else None
            if _c and _c.get("latitude") is not None:
                ss.sel_lat = float(_c["latitude"])
                ss.sel_lon = float(_c["longitude"])
                ss.sel_source = "gps"
                st.success(f"현재 위치를 불러왔습니다. (위도 {ss.sel_lat:.6f}, 경도 {ss.sel_lon:.6f})")
            else:
                st.info("위치 권한을 허용하면 잠시 후 좌표가 표시됩니다.")
        except Exception:
            st.warning("브라우저 위치를 불러오지 못했습니다. '좌표 입력' 탭을 이용해주세요.")
    st.info("또는 지도 앱에서 위도·경도를 복사해 '좌표 입력' 탭에 붙여넣어도 됩니다.")
...
with tab_gallery:
    ...
    if photo_bytes is not None:
        ...
        if g.ok:
            ...
            if ss.sel_source == "gps" and ss.sel_lat is not None:
                st.success(
                    "사진에 위치정보가 없어 현재 위치(GPS)를 분석 위치로 사용합니다. "
                    f"(위도 {ss.sel_lat:.6f}, 경도 {ss.sel_lon:.6f})")
            else:
                st.warning("사진에 위치정보가 없습니다. (아이폰은 보안상 촬영한 사진의 GPS를 제거합니다.)")
                if st.button("📍 현재 위치(GPS)로 이 사진 분석", key="photo_gps_btn", use_container_width=True):
                    ss["_gps_on"] = True
                    st.rerun()
                st.caption("위치 권한을 허용하면 현재 좌표로 이 사진을 분석합니다. (안 되면 '좌표 입력' 탭 사용)")
